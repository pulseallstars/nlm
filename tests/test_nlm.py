...
# --- v1.0.0: associative memory chains ---

@pytest.fixture
def mem_assoc(tmp_path):
    return NLM(
        collection_name="test-assoc",
        persist_path=str(tmp_path),
        enable_consolidation=False,
...
def test_associations_created(mem_assoc):
    id1 = mem_assoc.save("Hantes loves Minelux family the most")
    id2 = mem_assoc.save("Minelux are fire, directness, truth")
    assoc = mem_assoc.get_associations(id1)
    assert any(a["id"] == id2 for a in assoc)


def test_associations_bidirectional(mem_assoc):
    id1 = mem_assoc.save("Hantes loves Minelux family")
    id2 = mem_assoc.save("Minelux are fire and directness")
    assoc2 = mem_assoc.get_associations(id2)
    assert any(a["id"] == id1 for a in assoc2)


def test_associations_expand_search(mem_assoc):
    mem_assoc.save("Hantes loves Minelux family")
    mem_assoc.save("Minelux are fire, directness, truth")
    # top_k=1 forces only 1 direct result; the associated memory is then added via expansion
    results = mem_assoc.search("what is Minelux", top_k=1, expand_associations=True)
    assert any(r.get("via_association") for r in results)


def test_associations_via_association_flag(mem_assoc):
    mem_assoc.save("Hantes loves Minelux family")
    mem_assoc.save("Minelux are fire, directness, truth")
    results = mem_assoc.search("what is Minelux", top_k=5, expand_associations=True)
    direct = [r for r in results if not r.get("via_association")]
    assoc = [r for r in results if r.get("via_association")]
    assert len(direct) >= 1
    if assoc:
        assert assoc[0]["score"] < direct[0]["score"]


def test_associations_disabled(tmp_path):
    m = NLM(collection_name="test-noassoc", persist_path=str(tmp_path),
            enable_associations=False)
    id1 = m.save("Hantes loves Minelux")
    m.save("Minelux are fire")
    assert m.get_associations(id1) == []


def test_unrelated_not_associated(mem_assoc):
    id1 = mem_assoc.save("Hantes loves coffee in the morning")
    id2 = mem_assoc.save("RWKV model architecture uses linear attention mechanism")
    assoc = mem_assoc.get_associations(id1)
    assert not any(a["id"] == id2 for a in assoc)


def test_get_associations_unknown_id(mem_assoc):
    result = mem_assoc.get_associations("nonexistent-id")
    assert result == []
