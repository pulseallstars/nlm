...
def parse_iso(s: str) -> datetime:
    ...
    Accepts the 'Z' suffix (Python 3.9/3.10 fromisoformat rejects it),
    promotes naive datetimes to UTC, and raises ValueError on empty input.
    """
    if not s:
...
def specificity_score(text: str) -> float:
    ...
    Unicode-aware — str.isupper() recognises Cyrillic/Greek capitals, so
    Ukrainian proper nouns ("Хантес", "Чернівці") count the same as English.
    """
    ...
    # Proper nouns: capitalized words after the first token
